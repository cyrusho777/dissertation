...
import (
	...
	"sigs.k8s.io/scheduler-plugins/pkg/multiresource"

	// Ensure scheme package is initialized.
	_ "sigs.k8s.io/scheduler-plugins/apis/config/scheme"
)

// multiResourcePluginFactory is a simple wrapper that conforms to the PluginFactory interface
func multiResourcePluginFactory(ctx context.Context, obj runtime.Object, handle framework.Handle) (framework.Plugin, error) {
	return multiresource.New(handle)
}
...
func main() {
	...
	command := app.NewSchedulerCommand(
		...
		// Register our MultiResource plugin using the local factory function
		app.WithPlugin(multiresource.Name, multiResourcePluginFactory),
	)
